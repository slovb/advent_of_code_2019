...
class PortalNode:
    ...
    def append(self, node, steps, levelmod):
        self.adjacent.append((node, steps, levelmod))
...
def read(filename):
    ...
    return nodes, nodes[processedPortals['start']], \
        nodes[processedPortals['end']]


def main(filename):
    nodes, start, end = read(filename)
    network = {p: PortalNode() for p in nodes}
    pnStart = network[start.p]
    pnEnd = network[end.p]
    pnEnd.isEnd = True
    for np in nodes:
        if not nodes[np].isPortal:
            continue
        mem = set([np])
        queue = [(0, np)]
        while len(queue) > 0:
            step, qp = queue.pop()
            for a, mod in nodes[qp].adjacent:
                if a.p in mem:
                    continue
                if mod != 0:
                    network[np].append(network[a.p], step + 1, mod)
                else:
                    queue.append((step + 1, a.p))
    print(pnStart.adjacent)
