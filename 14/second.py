...
def simulate(craft, priority, fuel_req):
    oreReq = 0
    requirements = [[fuel_req, 'FUEL']]
...
def solve(data):
    ...
    oreLimit = 10**12
    fuelReq = 1
    oreReq = simulate(craft, priority, fuelReq)
    while oreReq < oreLimit:
        avg = oreReq // fuelReq
        if 2*oreReq < oreLimit:
            fuelReq *= 2
        elif oreReq + 10000 * avg < oreLimit:
            fuelReq += 10000
        else:
            fuelReq += 1
        oreReq = simulate(craft, priority, fuelReq)
        print(fuelReq, oreReq)
    return fuelReq - 1
