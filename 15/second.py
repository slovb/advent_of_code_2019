...
def max_distance(state, pos):
    memory = {pos: 0}
    options = [(pos, 0)]
    while len(options) > 0:
        opt, m = options.pop(0)
        for d in range(1, 5):
            p = update_position(opt, d)
            if state[p] == 1 and (p not in memory or memory[p] > m + 1):
                memory[p] = m + 1
                options.append((p, m + 1))
    m = 0
    for p in memory:
        m = max(m, memory[p])
    return m
...
def solve(memory):
    ...
    return max_distance(state, oxygen_pos)
